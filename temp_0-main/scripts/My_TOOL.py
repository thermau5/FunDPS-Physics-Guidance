...
def plotline(*args, x_change=0, figsize=(8.5, 6), xname='x', yname='y', title='y~x', have_x=True, label=None,
             linewidth=1.5, overlap=0, xnum=9, ynum=0, xscale='linear', yscale='linear',
        ...
        if n_draw == 1:
            ...
            if overlap == 0:
                for i in range(n_draw - 1):
                    plt.plot(data_draw[-1][:len(data_draw[i])], data_draw[i], linestyle='-', label=label[i],
                             linewidth=linewidth)
            else:
                for i in range(n_draw - 1):
                    plt.plot(data_draw[-1][:len(data_draw[i])], data_draw[i], linestyle='-', label=label[i],
                             linewidth=linewidth, dashes=(i + 2, 1 + 0.2 * i))
...
def plotheat(x, y, z, figsize=None, xname='x', yname='y', barname='Function Value', title='f(x,y)', label=None,
             linewidth=1.5, overlap=0, xnum=9, ynum=0,
    ...
    heatmap = plt.pcolormesh(data_draw[0], data_draw[1], data_draw[2], cmap='viridis', shading='auto', vmin=vmin,
                             vmax=vmax)  # cmap=viridis: green=0; bwr: white=0
...
class split_input_chkpt_advanced(nn.Module):
    def __init__(self, f, chkpt=0, splt=0, splt_bp=None, dim=2, dvc_cpt='cuda', dvc_chkpt='cpu', no_splt=False,
                 fwd_dtype=None):
        super().__init__()
        '''
        Parameters
