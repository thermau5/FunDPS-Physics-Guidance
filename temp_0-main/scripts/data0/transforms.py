...
class PositionalEmbedding():
    def __init__(self, grid_boundaries, channel_dim,dim_pde):
        self.grid_boundaries = grid_boundaries
        self.channel_dim = channel_dim
        self._grid = None
        self.dim_pde=dim_pde

    def grid(self, data):#data: 1*gx*gy  (one element of n*1*gx*gy)
        if self._grid is None:
            self._grid = get_grid_positional_encoding(data, 
                                                      grid_boundaries=self.grid_boundaries,
                                                      channel_dim=self.channel_dim,
                                                      dim_pde=self.dim_pde) #1*1*gx*gy*
            for i in range(len(self._grid)): ##0225 add
                self._grid[i]=self._grid[i].squeeze(self.channel_dim)

        return self._grid

    def __call__(self, data):
        gridd = self.grid(data)
        # print("!!!!!")
        # griddd=gridd.copy()
        # for i in range(len(gridd)):
        #     wcw.sss(gridd[i])
        #     gridd[i]=gridd[i].squeeze(self.channel_dim)
        #     wcw.sss(gridd[i])
        # aaa=[data]+gridd
        # wcw.sss(aaa)
        # for i in aaa:
        #     wcw.sss(i)
        return torch.cat([data]+gridd,dim=0)#(feature+dim)*gx*gy*..
