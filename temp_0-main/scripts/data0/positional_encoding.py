...
def get_grid_positional_encoding(input_tensor, grid_boundaries=[[0,1],[0,1]], channel_dim=0,dim_pde=1):
    ...
    for i in range(dim_pde):
        ## only for time-pde with shape T,X,... 24/8/31
        slic=slice(1,None) if i==0 else slice(None,-1)
        gridd.append(torch.linspace(grid_boundaries[i][0],grid_boundaries[i][1],shape[i]+1)[slic].to(device))
        ##
