...
s = 128
sub = 1
N = 50000  # Generate 50,000 samples for training (or 1000 for testing)
...
record_steps = 50  # Generate 50 timesteps, extract 25th and 50th
bsize = 200

for i in range(50):  # Generate 50 files of 1000 samples each = 50,000 total
    # Only store the 2 timesteps we need (25th and 50th)
    u_selected = torch.zeros(1000, s, s, 2)  # Only 2 timesteps instead of 50
    c = 0
    t0 = default_timer()
    for j in range(1000 // bsize):
        print(f"File {i+1}/50, Batch: {j}")
        w0 = GRF.sample(bsize)
        sol, sol_t = navier_stokes_2d(w0, f, 1e-3, 5, 1e-3, record_steps)  # T=5 for physical time = 5
        # Extract only 25th (index 24) and 50th (index 49) timesteps
        u_selected[c : (c + bsize), :, :, 0] = sol[:, :, :, 24]  # 25th timestep
        u_selected[c : (c + bsize), :, :, 1] = sol[:, :, :, 49]  # 50th timestep
        c += bsize
        t1 = default_timer()
        print(f"  Progress: {c}/1000, Time: {t1 - t0:.2f}s")
    
    filename = f"ns-nonbounded_{i+1}.mat"
    # Save with selected timesteps only
    ...
    print(f"Saved: {filename} ({u_selected.numel() * 8 / 1024**2:.1f} MB)")
    del u_selected
    del sol

print("\n" + "="*50)
print("TRAINING DATA GENERATION COMPLETE!")
print("Generated 50 files x 1000 samples = 50,000 training samples")
print("Each file contains 25th and 50th timesteps")
print("Move these files to: data/DiffPDE/training/ns-nonbounded_new/")
print("="*50)
